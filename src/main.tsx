...
function CaseStudy({ project, projects, onBack, onNext }: { project: Project; projects: Project[]; onBack: () => void; onNext: () => void }) {
  ...
  return <section className={`case-study ${isMotion ? 'motion-case' : 'creative-case'}`} aria-label={`${project.title} case study`}>
    <header className="case-nav"><button data-cursor="OPEN" onClick={() => transition(onBack)}>← BACK TO {isMotion ? 'MOTION' : 'CREATIVE'} ARCHIVE</button><span>CASE STUDY / {project.type.toUpperCase()}</span><button data-cursor="OPEN" onClick={onNext}>NEXT <Arrow /></button></header>
    <div className="case-hero"><p>SELECTED PROJECT / 2026</p><h1>{project.title}<br /><em>{project.type}.</em></h1><div className="case-media" style={{ viewTransitionName: `media-${project.kind}` }}>{isMotion ? <video src={`/assets/videos/${project.asset}`} autoPlay muted loop playsInline /> : <img src={`/assets/images/${project.asset}`} alt="" />}</div></div>
    ...
    <div className="case-gallery"><div className="gallery-lead">{isMotion ? <video src={`/assets/videos/${project.asset}`} autoPlay muted loop playsInline /> : <img src={`/assets/images/${project.asset}`} alt="" />}</div><div className="gallery-copy"><p>GALLERY / MOTION PREVIEW</p><h2>Every frame earns<br />its <em>place.</em></h2><div className="software"><span>SOFTWARE</span><b>{isMotion ? 'AFTER EFFECTS · PREMIERE PRO · PHOTOSHOP' : 'PHOTOSHOP · ILLUSTRATOR · FIGMA'}</b></div></div></div>
...
function ProjectArchive({ kind, onClose }: { kind: 'creative' | 'motion'; onClose: () => void }) {
  ...
  return <section className={`project-archive ${kind === 'creative' ? 'creative-archive' : 'motion-archive'}`}><header className="archive-top"><button data-cursor="OPEN" onClick={() => transition(onClose)}>← CLOSE</button><p>{kind === 'creative' ? 'CREATIVE DESIGN' : 'MOTION GRAPHICS'} / SELECTED ARCHIVE</p><span>{String(projects.length).padStart(2, '0')} PROJECTS</span></header><div className="archive-title"><h1>{kind === 'creative' ? <>Creative<br /><em>Design.</em></> : <>Motion<br /><em>Graphics.</em></>}</h1><p>A full archive of designed images, identities and worlds made to hold attention.</p></div><div className="archive-grid">{projects.map((project, index) => <button className="archive-project" data-cursor={kind === 'motion' ? 'PLAY' : 'VIEW'} key={project.asset} onClick={() => transition(() => setSelected(project))}><div className="archive-media">{kind === 'motion' ? <video src={`/assets/videos/${project.asset}`} muted autoPlay loop playsInline preload="metadata" /> : <img src={`/assets/images/${project.asset}`} alt="" />}</div><span>0{index + 1} / {project.type}</span><strong>{project.title}</strong><Arrow /></button>)}</div></section>
}
      ...
      <section id="projects" className="projects">
        ...
        <div className="feature-categories">
          ...
          <button className="feature-card collection-feature illustration-feature" data-cursor="VIEW ALL" onClick={() => transition(() => setArchive('illustrations'))}>
            <div className="feature-media media-contain"><img src="/assets/illustrations/iron%20man%20illustration.jpeg" alt="Iron Man illustration" /><div className="feature-glass" /></div>
          ...
          <button className="feature-card collection-feature motion-graphics-feature" data-cursor="VIEW ALL" onClick={() => transition(() => setArchive('motion-graphics'))}>
            <div className="feature-media media-contain"><video src="/assets/motion%20graphics/devansh%20bansal%27s%20motion%20graphic%20portfolio%20-%201080.mp4" muted playsInline preload="metadata" /><div className="feature-glass" /></div>
